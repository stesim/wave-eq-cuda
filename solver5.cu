...
void Solver5::solve()
{
	...
	CudaHelper5::calculateFirstStep<<<gpudom / threads, threads>>>(
			dt,	h, ip, matInner, matLeft, matRight,	d_w, d_u, d_z );

	CudaHelper5::synchronizeResults<<<gpudom / threads, threads>>>(
			ip,
			d_z,
			d_w );
	...
	{
		CudaHelper5::calculateNSteps<<<gpudom / threads, threads>>>(
...
SpDiaMat Solver5::gpuAllocFDMatrixInner( double l2, unsigned int ip )
{
...
SpDiaMat Solver5::gpuAllocFDMatrixLeft( double l2, unsigned int ip )
{
	unsigned int diagSize = ip - 1;
...
SpDiaMat Solver5::gpuAllocFDMatrixRight( double l2, unsigned int ip )
{
	unsigned int diagSize = ip - 1;
...
void Solver5::gpuFreeFDMatrix( SpDiaMat& mat )
{
